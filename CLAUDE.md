...
- [x] P3 Kalender tonal an Pollen (Screenshot-Delta-Analyse 04.07.) - KOMPLETT mit P3e (v0.1.0379):
  ...
  - [x] P3e Monatsgrid Ton-Abgleich + Produktentscheidung (v0.1.0379): BEFUND: Die sichtbare "Monat"-Ansicht rendert miniMonthHtml() -> .cal-premium-mini-grid (NICHT das cfx-*-Vollgrid aus calendar-logic.js; das ist im Premium-Kalender ausgeblendet). Produktentscheidung: PUNKTEZELLEN behalten (Charta: minimalistisch/Apple-Notion; Detailzellen wuerden das Mini-Grid ueberladen; die cfx-Detailzellen-Spez betrifft das versteckte Vollgrid). Problem: Das Mini-Grid war fuer eine schmale Seitenspalte gebaut (999px-Pille = Kreis bei ~40px-Zellen), im Vollbreiten-Monatsmodus wurden Zellen aber 177px breit -> ausgewaehlter Tag = gestreckte Pille, Zahlen schwammen in kurzen Zellen. Fix (calendarPanels.css, Basis direkt umgeschrieben): Monatszellen als echte Kalenderzellen im Stil der Wochenleiste (1:1 gemessen: min-height 60/mobil 52, Radius 14, Border/BG .075/.035, Zahl oben links, Selected = gruener Verlauf + .72-Border statt Pille, has-Dot 7px gruen); grid-template-columns repeat(7,minmax(0,1fr)) behebt mobilen So-Spalten-Ueberlauf. Desktop+Mobil browser-verifiziert. Damit ist F/P3 komplett.
...
- Produktentscheidung (Punkte- vs. Detailzellen): PUNKTEZELLEN beibehalten. Begruendung: Charta (minimalistisch, Apple/Notion, viel Weissraum); Termin-Text in einem Mini-Monat wuerde ueberladen; die Charta-Detailzellen-Spez betrifft das separate, versteckte cfx-Vollgrid. Nutzer hatte "mach gerne weiter" gesagt -> Entscheidung mit Begruendung getroffen statt erneut gefragt.
- WURZELPROBLEM (am DOM gemessen): Das Mini-Grid war fuer eine schmale Seitenspalte gebaut (Zellen height 38, border-radius 999px = Kreis bei ~40px-Breite). Im Vollbreiten-Monatsmodus rendern die Zellen aber 177px breit -> der ausgewaehlte Tag wurde eine gestreckte 999px-Pille, die Zahlen schwammen in kurzen 38px-Zellen. Kein fertiger Kalender.
- Fix (features/calendar/calendarPanels.css, kanonische Basis Z.36 DIREKT umgeschrieben - keine neue Override-Schicht): Monatszellen als echte Kalenderzellen im Stil der bereits Pollen-angeglichenen Wochenleiste (Werte 1:1 von .cal-premium-week-day gemessen): min-height 60 (mobil 52), Radius 14, Border rgba(255,255,255,.075), BG rgba(255,255,255,.035), Zahl oben links (flex column), Hover-Lift; today = gruene Zahl; Selected = gruener Verlauf linear(rgba(74,222,128,.13)->.035) + Border rgba(74,222,128,.72) + weisse Zahl (statt solider Pille); has-Dot 7px gruen unten. Wochentagskoepfe linksbuendig .5/12px. grid-template-columns repeat(7,minmax(0,1fr)) behebt den mobilen So-Spalten-Ueberlauf; mobiler P3e-Block (min-height 52, padding 8/6, 14px) am Dateiende.
- Browser-verifiziert Desktop 1440 (Zellen 177x60, Selected gerundete Zelle nicht Pille) + Mobil 375 (7 Spalten vollstaendig sichtbar, kein Ueberlauf: docScrollW == Viewport; Zellen 39x52); Konsole fehlerfrei. Woche/Hero/Agenda unveraendert (nur .cal-premium-mini-grid beruehrt).
- Damit ist Abschnitt F/P3 (Kalender tonal an Pollen) KOMPLETT: P3a Backdrop, P3b Hero, P3c Switch, P3d Agenda, P3e Monat. Offen in F nur noch P2 (Challenges) und P4 (Mobile-Pass).
- Nur CSS + Versions-Strings (byte-sicher via Python).
...
- Ziel 1:1 am lebenden pollen-neo-hero-insights gemessen (echtes Insight-Markup inkl. pollen-neo-status-icon injiziert, Mobil 375): Zeilen 38px hoch, Streifen 47px (8px Pad oben), Trennlinien rgba(244,247,244,.10), Icon-Chips 18px rund mit Tint rgba(74,222,128,.14), Zeilen-Grid 18px-Spalte/gap 7/pad 0 8, erste Zelle ohne Links-Padding, letzte ohne Trennlinie.
- Umsetzung (styles/appShell.css, P3b-Kalenderzweig, Mobil-Media): cal-premium-hero-side/-row/-row-icon exakt auf diese Werte; SVG in den Chips 12px. Vorher: Zeilen 44px, Linien rgba(255,255,255,.07/.09), Chips 26px weiss-getint.
- Kalender mobil nachgemessen: alle Werte identisch zu Pollen (Zeile 38/Streifen 47/Chip 18x18/Linien .10); Hero weiter exakt 216px. Desktop gegengeprueft unveraendert (238px, 26px-Chips); Konsole fehlerfrei.
- Nur CSS + Versions-Strings (byte-sicher via Python).
- Cache-Busting ?v=0.1.0375.
- Geaendert: `styles/appShell.css`, `features/settings/settingsPanel.js`, `features/pollen/pollenView.js`, `index.html`, `version.json`, `CLAUDE.md`, `CHANGELOG.md`.

## Version 0.1.0374 - P3a-Fix: Kalender-Backdrop wirkte nie (toter Selektor) + Hero-Hoehe exakt
- Nutzer-Feedback zu 0372 (live): "Farbe passt noch nicht, Hoehe ebenfalls, mobil auch nicht". Hinweis: Live lief zu dem Zeitpunkt noch 0372, Repo war schon auf 0373 (Pages-Deploy hinkte nach) - Text-Stack-Punkte waren durch 0373 teilweise schon adressiert.
- WURZELFUND Farbe: Die P3a-Backdrop-Regel aus v0370 zielte auf #calendar-view - dieses Element existiert weder in index.html noch im JS. Die Regel war von Anfang an wirkungslos; der Kalender lag auf flachem Dunkel, waehrend bei Pollen der Radial-Backdrop durch die halbtransparente Hero-Karte scheint. Deshalb wirkte die Kalender-Karte dunkler/toter als das Pollen-Vorbild.
- Fix (features/calendar/calendarPanels.css): Backdrop auf den realen Traeger #calendar-premium-view (::before, inset 0, Inhalte z-index 1, min-height 100%). Werte NICHT aus dem Quelltext kopiert, sondern am lebenden #pollen-view::before gemessen (spaete Generation gewinnt dort): linear #03090c + radial(circle at 12% 0%, rgba(19,104,60,.13), transparent 30%). #content ist in beiden Views transparent, dahinter #main-app #04090e - identischer Rahmen, kein Override noetig (geprueft, bewusst KEINE Regel ergaenzt).
- Hoehe: Kalender-Hero renderte 246-248px statt Pollen 238px - Ursache war die 190px-Uhr-Illustration (1:1-Ratio -> 190px hoch + 56 Padding). Pollen-Illustration rendert real 168x168 (gemessen, viewBox 130x130). Fix: Illustration auf 168px -> Hero rendert exakt 238px (nachgemessen). Mobil exakt 216px (nachgemessen).
- Browser-verifiziert Desktop 1440 + Mobil 375 (Backdrop aktiv, Hoehen exakt, Konsole fehlerfrei); nur Versions-Strings im JS (byte-sicher via Python).
...
- JS (features/calendar/calendarPanels.js): neue Funktion heroIconSvg (Uhr/Schere/Sonnenschirm als stroke=currentColor Linien-SVGs, 24er viewBox, 14px); calendarHeroRow laesst Icons, die mit '<' beginnen, roh durch (sonst weiter esc()); die drei Hero-Zeilen nutzen die SVGs statt Emojis. Uhr-Illustration (calHeroArtSvg) von #7DE6AB/#13251A auf #4ade80/#0f241b.
- CSS (styles/appShell.css): neuer, klar markierter Kalender-Zweig "P3b-2" am Dateiende (nach den 0290c/d-Locks, gewinnt per Reihenfolge bei gleicher Spezifitaet): Desktop >=761px Grid minmax(0,1fr) 190px minmax(340px,520px) + gap 26 + min-height 238 + padding 28px 30px + Radius 28 + Pollen-Schatten; main-Padding 0; Illustration 190px; Side-Padding links 20px; Zeilen 26px-Ikonspalte/gap 12/padding 10 0; Icon-Chips 26px rund rgba(255,255,255,.06); Wert 14px/900; mobil Radius 26, Label 10px/900, Wert 11px/950.
- Verifiziert im Browser (Desktop 1440: Hero-Spalten rendern exakt 496/190/520 wie der gemessene Pollen-Hero; Mobil 375; Konsole fehlerfrei = JS-Syntax ok, kein node verfuegbar); Dashboard-Hero gegengeprueft: unveraendert (Gradient/Radius Standard-Tokens).
- Fundstelle dokumentiert: appShell.css traegt seit mind. v0370 eine ueberzaehlige schliessende Klammer (vorbestehend, Browser-tolerant); bei P4-Cleanup beheben.
- Versions-Bump byte-sicher via Python (read_bytes/replace/write_bytes).
...
- Geprueft: `node --check`; headless Logik-Tests (Slot/Dedupe).
...
- Geaendert/Neu: `features/settings/settingsPanel.js`, `features/settings/settingsPanel.css`, `features/pollen/pollenView.js`, `index.html`, `CLAUDE.md`, `CHANGELOG.md`.
...
- Geprueft: `node --check`; Render-Vorschau.
...
- Toter Helfer `dataAuditChip` entfernt; neue Render-Helfer `auditStat`/`auditDiag` + Icon-Helfer. Styling modular in `features/settings/settingsPanel.css` ueber `--st-*`-Tokens (scope `#settings-view`, theme-fest).
- "Settings-Keys" wird im Entwurf nicht mehr als Chip gezeigt (Charta: minimalistisch); Wert wird im Report weiterhin berechnet.
- Keine Kalender-, Dashboard-, Challenge-, Sync- oder Firebase-Logik beruehrt.
- Cache-Busting ?v=0.1.0337.
- Geaendert: `features/settings/settingsPanel.js`, `features/settings/settingsPanel.css`, `features/pollen/pollenView.js`, `index.html` (Cache-Busting ?v=0.1.0337), `CLAUDE.md`, `CHANGELOG.md`.
- Geprueft: `node --check` (settingsPanel.js, pollenView.js).
...
- Geaendert: `features/settings/settingsPanel.js`, `index.html`, `core/calendar/calendarOwnerAudit.js`, `features/pollen/pollenView.js`, `CLAUDE.md`, `CHANGELOG.md`.
...
- Geaendert: `features/settings/settingsPanel.js`, `features/settings/settingsPanel.css`, `index.html`, `core/calendar/calendarOwnerAudit.js`, `features/pollen/pollenView.js`, `CLAUDE.md`, `CHANGELOG.md`.
- Geprueft: `node --check`, `scripts/auditDataModel.mjs`, Asset-/Versionspruefung.
...
- Keine Kalender-Bereinigung in dieser Version, keine Layout-/Daten-/Sync-/Firebase-Aenderung.
