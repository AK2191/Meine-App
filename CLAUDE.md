## Version 0.1.0273
- **Bug: Während eines laufenden GitHub-Updates/Rollbacks sprang der Lade-/Fortschrittsblock ("Update wird veröffentlicht…") visuell über die Tabs "Update"/"Verlauf" – im Ruhezustand stand die ZIP-Karte aber unter den Tabs, wie erwartet.**
- **Root Cause:** In `githubUpdateBody()` (`features/settings/settingsPanel.js`) wurde `actionPanel` (das den laufenden Update-/Rollback-Fortschritt rendert) fix **vor** `githubTabBar()` in den HTML-String eingefügt – unabhängig vom aktiven Tab. Im Ruhezustand ist `actionPanel` leer, daher unauffällig; sobald ein Update läuft, erschien der Block strukturell oberhalb der Tab-Leiste statt im Update-Tab an der Stelle der ZIP-Karte.
- **Fix:** `actionPanel` wird jetzt innerhalb von `updatePane` direkt über `githubFileOverview()` (an der Stelle der bisherigen `statusLine`/`checks`) gerendert, statt global vor der Tab-Leiste. Reihenfolge jetzt in jedem Zustand konsistent: Freigabe-Code → Tabs (fest) → Tabpane (Dropzone → Status → Fortschritt → Datei-Übersicht → Button).
- Geändert: `features/settings/settingsPanel.js`, Funktion `githubUpdateBody()` (reine Reihenfolge der HTML-Bausteine, keine neue Logik, kein State, kein CSS). Keine Änderung an Worker-Polling, State-Machine, Rollback-Logik, Kalender, Dashboard oder Challenges.
- `node --check` für `features/settings/settingsPanel.js` grün.
