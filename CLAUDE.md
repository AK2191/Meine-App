...
| `connectToGoogle` + `handleGoogleLogin` | GIS-Popup friert auf GitHub Pages ein (window.closed-Polling-Bug) | Beide nutzen OAuth 2.0 Implicit Redirect (kein Popup). Redirect-URI `https://ak2191.github.io/Meine-App/` muss in Google Cloud Console unter "Autorisierte Weiterleitungs-URIs" stehen. `handleGoogleOAuthRedirect()` liest Token aus URL-Hash (state=main_login oder gcal_connect). NIE wieder GIS TokenClient/requestAccessToken verwenden! `handleGoogleLogin()` + `firebaseMobileLoginFallback()` nutzen jetzt Firebase `signInWithRedirect(GoogleAuthProvider)` (NICHT mehr den manuellen Implicit-Flow – der lieferte ohne registrierte redirect_uri keinen Token → Login-Loop). Calendar-Token kommt via `credentialFromResult()`. `connectToGoogle()` (Settings) nutzt weiterhin Implicit-Redirect mit state=gcal_connect – nur relevant wenn Calendar separat neu verbunden wird; falls auch das keinen Token liefert, ebenfalls auf Firebase-Credential umstellen. NIE wieder GIS TokenClient/requestAccessToken/signInWithPopup (COOP). Voraussetzung Firebase-Login: ak2191.github.io in Firebase Console → Authentication → Authorized domains. (Fix 2026-05-26) |
