...
(function(){
  ...
  function injectCss(){
    ...
    st.textContent=`
      ...
      /* Dashboard ruhiger: Datum direkt beim Text, nicht ganz rechts */
      #dash-grid{grid-template-columns:1fr!important;max-width:980px!important}
      .change-dash-row{justify-content:flex-start!important;gap:10px!important;min-height:44px!important;padding:9px 16px!important}
      .change-dash-date{flex:0 0 auto;font-size:11px;font-weight:850;color:var(--grn);background:var(--grn-d);border:1px solid rgba(22,163,74,.14);padding:4px 8px;border-radius:999px;white-space:nowrap;font-family:var(--mono);min-width:74px;text-align:center}
      .change-dash-date.today{background:var(--acc-d);color:var(--acc);border-color:rgba(45,106,79,.18)}
      .change-dash-date.holiday{background:var(--amb-d);color:var(--amb);border-color:rgba(217,119,6,.20)}
      .change-dash-badge{display:none!important}
      .change-dash-title{display:flex;align-items:center;gap:7px;min-width:0}.change-dash-title-text{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .change-holiday-pill{display:inline-flex;align-items:center;flex:0 0 auto;font-size:10px;font-weight:850;color:var(--amb);background:var(--amb-d);border:1px solid rgba(217,119,6,.16);padding:2px 6px;border-radius:999px}
      .change-dash-meta{font-size:11.5px;color:var(--t4);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      /* Kalender: Heute subtil, Termine/Zeiträume vorne */
      #month-grid .day-cell.today{background:rgba(45,106,79,.045)!important;box-shadow:inset 0 0 0 1px rgba(45,106,79,.16)!important}
      #month-grid .day-cell.today .day-num-wrap{background:transparent!important;box-shadow:none!important;border-radius:6px!important;position:relative!important;z-index:2!important;width:auto!important;height:auto!important;min-width:18px!important;padding:0 2px!important}
      #month-grid .day-cell.today .day-num{color:var(--acc)!important;font-weight:900!important}
      #month-grid .holiday-line{position:relative!important;z-index:2!important;display:inline-flex!important;max-width:calc(100% - 26px)!important;margin-left:4px!important;border-radius:999px!important;background:var(--amb-d)!important;border:1px solid rgba(217,119,6,.18)!important;color:var(--amb)!important;font-size:9px!important;font-weight:800!important;padding:2px 6px!important;vertical-align:top!important}
      #month-grid .ev-chip{position:relative!important;z-index:9!important}
      .change-range-bar{z-index:8!important;top:30px!important}
    `;
  }
  ...
  function buildCalendarRows(){
    ...
    return combined.map(item=>{const dkey=String(item.date||item.startDate||'').slice(0,10);const h=getHolidayNameFor(dkey);const isT=dkey===todayKey();return item.isHoliday?`<div class="change-dash-row"><div class="change-dash-icon holiday">🎌</div><span class="change-dash-date holiday ${isT?'today':''}">${esc(isT?'Heute':deShort(item.date))}</span><div class="change-dash-main"><div class="change-dash-title"><span class="change-dash-title-text">${esc(clean(item.title))}</span><span class="change-holiday-pill">Feiertag</span></div><div class="change-dash-meta">Feiertag</div></div></div>`:`<div class="change-dash-row" onclick="window.__changeOpenDashEvent&&window.__changeOpenDashEvent('${esc(item.id)}')"><div class="change-dash-icon">📅</div><span class="change-dash-date ${isT?'today':''}">${esc(isT?'Heute':item.badge)}</span><div class="change-dash-main"><div class="change-dash-title"><span class="change-dash-title-text">${esc(item.title)}</span></div>${h?`<div class="change-dash-meta">🎌 ${esc(h)}</div>`:''}</div></div>`}).join('');
  }
