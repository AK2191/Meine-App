 ...
 * ════════════════════════════════════════════════════════
 * 40 Sport-Übungen · 7 täglich · 2 feste Optionale
 * ════════════════════════════════════════════════════════
 */
(function(){
  'use strict';
  ...
  /* ══════════════════════════════════════════
     DARK-MODE FIX für btn-undo & challenge-done
  ══════════════════════════════════════════ */
  (function injectStyle(){
    var id = 'challenges-fix-style';
    if(document.getElementById(id)) return;
    var s = document.createElement('style');
    s.id = id;
    s.textContent = [
      /* Erledigt-Zeile: dunkler Hintergrund im Dark Mode */
      '[data-theme="dark"] .challenge-done {',
      '  background: rgba(22,163,74,.10) !important;',
      '  border-left: 3px solid rgba(74,222,128,.4) !important;',
      '}',
      /* Zurücksetzen-Button: immer rot, nie weiß */
      '.btn-undo {',
  ...
  /* ══════════════════════════════════════════
     POOL — 40 Sport-Übungen mit YouTube-Links
  ══════════════════════════════════════════ */
  ...
  /* Feste Optionale — immer am Ende, unverändert */
  var OPTIONAL = [
  ...
  function safeUrl(u){
    if(!u) return '';
    try{ var x = new URL(u); return (x.protocol==='https:'||x.protocol==='http:') ? u : ''; }
    catch(e){ return ''; }
  }

  /* 7 deterministisch per Datum aus Pool */
  ...
  function pointsForDate(dk, pid){
    var td = String(dk || '').slice(0,10);
    var me = (pid || myId()).toLowerCase();
  ...
  window.completeChallenge = function(id){
    ...
    // Firestore: nur change_completions
    try{
    ...
    }catch(e){ console.warn('[Challenges] Firestore:', e); }

    if(typeof toast==='function') toast('+'+pts+' Punkte ✓','ok');

    // Glocke: Challenge-Benachrichtigung als gelesen markieren
    // → verhindert dass die Glocke nach Erledigen hochzählt
    try{
      ...
      // Bell-Indikator aktualisieren (nicht checkNotifications — das würde wieder aufmachen)
  ...
  function renderChallenges(){
    ...
    function chItem(ch){
      ...
      return '<div class="challenge-item'+(done?' challenge-done':'')+'" style="pointer-events:auto">'
        ...
        +link
        +'</div>'
        +'<span class="points-pill" style="flex-shrink:0">+'+pts+'</span>'
        +btn
        +'</div>';
    }

    /* 7 Pflicht-Challenges */
    var html = daily.map(chItem).join('');

    /* 2 Optionale — immer am Ende */
    ...
    try{
      ...
      var players = (window.challengePlayers || []).filter(function(p){
        ...
        if(!k || seen.has(k) || (window.isDemoMode === false && k==='demo@example.com')) return false;
  ...
  window.getChallengeDayStatus     = function(dk){
    var p = pointsForDate(dk, myId());
    return p > 0 ? {points:p, done:true, allDone:true} : null;
  };
  ...
  // window.challenges kompatibel halten
